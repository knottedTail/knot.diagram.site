...
\multirow{5}{120pt}{
	\centering
	\includegraphics[width=112pt]{../../../GIT/diagram.site/Diagrams/png/14_7_7.png}\\
...
\begin{tikzpicture}[x=20pt, y=17pt]
	...
	(C1D) edge (C1U) (C2U) edge (C2D) (C3D) edge (C3U) (C4U) edge (C4D) (C5D) edge (C5U) (C6U) edge (C6D) (C7D) edge (C7U) ;
	\end{tikzpicture} \\
\hhline{~~} \\& 
...
u^4+u^2- u+1\rangle \\
I^u_{2}&=\langle 
u^6+u^5+2 u^4+2 u^3+2 u^2+2 u+1\rangle \\
\\
\end{align*}
...
\centering \section*{I. $I^u_{1}= \langle u^4+u^2- u+1 \rangle$}
...
\flushright $a_{6}=$&$\begin{pmatrix}-1\\u^3+1\end{pmatrix}$\\ \flushright $a_{6}=$&$\begin{pmatrix}-1\\u^3+1\end{pmatrix}$\\&\end{tabular}
...
\flushright $a_{6}=$&$\begin{pmatrix}u^5+2 u^3+u+1\\u^5+u^3+u^2+u\end{pmatrix}$\\ \flushright $a_{6}=$&$\begin{pmatrix}u^5+2 u^3+u+1\\u^5+u^3+u^2+u\end{pmatrix}$\\&\end{tabular}
\flushleft \textbf{(ii) Obstruction class $= -1$}\\~\\
